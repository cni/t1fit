...
class UnwarpEpi(object):
    ...
    def prep_data(self, nifti1, nifti2, pe0=None, pe1=None, esp=0):
        ...
        #    pe_dir2 = -1
        if esp == 0: 
            ecsp1 = float([s for s in ni1.header.__getitem__('descrip').tostring().split(b';') if s.startswith(b'ec=')][0].decode().split('=')[1])
            ecsp2 = float([s for s in ni2.header.__getitem__('descrip').tostring().split(b';') if s.startswith(b'ec=')][0].decode().split('=')[1])
        else:
            ecsp1 = esp
            ecsp2 = esp 
        ...
        for f in [pe0, pe1]:
            if f!=None:
                ni = nb.load(f)
                if ni.header.get_data_shape()[2]%2:
                    im = ni.get_fdata()
...
if __name__ == '__main__':
    ...
    if args.b0map_flag:
        if not args.unwarp_t1map:  # if using B0map to correct unshuffled image
            ...
            command = './fsl-fmap-correction '+t1fit_base+'_t1.nii.gz '+b0map_magnitude+' '+b0map_frequency+' '+str(esp)+' '+unwarpdir+' '+t1fit_base+'_t1'
            print("command = {}".format(command))
            os.system(command)
